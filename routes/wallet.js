...
router.post('/:accountid/:type', function (req, res) {
    if (io == undefined) {
...
router.get('/:accountid', function (req, res) {
    ...
    for (var x = 0; x < debtIds.length; x++) {
        ...
        debts.push({ ID: debtIds[x], lender: lending[0], borrowDate: _date_borrow, expiryDate: _date_expiry, amount: lending[2] * Math.pow((1 + lending[3] * 0.01 * 0.083333), lending[4]), state: stateStr });
    }
    var _allLendings = [];
